...
delivery_note(frm) {
	// Trigger delivery note PDF download when delivery_note field is set
	if (frm.doc.delivery_note) {
		frappe.call({
			method: "tiger_integration.api.logo_sync.download_delivery_note_pdf",
			args: {
			...
			callback: function(r) {
				...
				var dialog = new frappe.ui.Dialog({
					title: result.op_result
						? __("Delivery Note PDF Download Successful")
						: __("Delivery Note PDF Download Failed"),
					fields: [
						{
							fieldtype: "HTML",
							fieldname: "steps_html",
