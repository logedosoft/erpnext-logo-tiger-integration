			...
			if existing_files:
				...
				if elogo_username and elogo_password and elogo_service_address:
					...
					if doctype == "Sales Invoice":
						query = "SELECT GUID, EINVOICE FROM {INVOICE} WHERE LOGICALREF = %(logo_ref_no)s AND TRCODE IN (7, 8)"
					elif doctype == "Delivery Note":
						query = "SELECT GUID FROM {STFICHE} WHERE LOGICALREF = %(logo_ref_no)s AND TRCODE IN (7, 8)"
